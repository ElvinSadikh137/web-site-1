...
const Update = ({ data, isOpen, onClose }) => {
    ...
    if (customerInfo) {
        ...
        const makePost = async (values) => {
            try {
                values.id=customerInfo.id;
                const response = await axios.post('http://localhost:8080/customers/updateCustomer', values);
        ...
        return (
            <Dialog open={isOpen} sx={{"& .MuiDialogContent-root":{backgroundColor:colors.black[800]}}}>
                <DialogTitle sx={{ backgroundColor: colors.greenAccent[700], color:colors.white[100] }}>Update Customer <CloseIcon sx={{ marginLeft: "390px" }} onClick={onClose} /></DialogTitle>
                <DialogContent sx={{ backgroundColor: colors.grey[600] }}>
                    <Formik
                        initialValues={{
                            ...
                            active: customerInfo.activeStatus,
